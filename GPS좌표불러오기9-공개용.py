...
def myf브이월드구주소로새주소및좌표검색(검색할지역,검색할번지,myv주소변환인증키): #data.go.kr 함수가 너무 구리고 인증키도 3개월마다 갱신하라고 해서 갈아탐
    ...
    try:    
        ...
        # 행정구역 =   result['response']['result']['items'][0]['address']['district'] 
...
def myf브이월드새주소로구주소및좌표검색(검색내용, 구색맞추기용,myv주소변환인증키): #data.go.kr 함수가 너무 구리고 인증키도 3개월마다 갱신하라고 해서 갈아탐
    ...
    """
        ...
        print("------오류가 있네요: "+검색내용)
    """
    return 구주소, 새주소, 우편번호, 위도, 경도, "" #행정구역


def myf주소로검색후자료넣기(주소):
    myv구새우위경 = myf브이월드구주소로새주소및좌표검색(주소,"",myv주소변환인증키) #구주소로 새주소 및 좌표 검색. 

    if myv구새우위경[0] == "":
        myv구새우위경 = myf브이월드새주소로구주소및좌표검색(주소,"",myv주소변환인증키)
    else:
        myv구새우위경 = myf브이월드새주소로구주소및좌표검색((myv구새우위경[1]),"",myv주소변환인증키) #새주소로 새주소 및 좌표 검색. 구주소로 해도 되긴하나, 새주소로 나오는 것이 더깔끔함
    ...
    sheet.cell(i,7).value = (myv구새우위경[5]) #행정구역
